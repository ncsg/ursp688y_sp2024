...
#Alanna Anderson's functions
def count_unique_available_bikes(df):
    return df[(df['is_reserved'] == 0) & (df['is_disabled'] == 0)]['name'].nunique()

def available_bikes_by_hour(df):
    return df.groupby(pd.Grouper(key='timestamp', freq='h', origin='start')).apply(count_unique_available_bikes)[:24]